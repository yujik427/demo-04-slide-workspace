...
**Addness Slide Studio** は、販売スライドを「同じ品質で何度も量産する」ためのワークスペース型 Web アプリ。原稿生成から PNG 出力までを1画面の4区構成で扱い、各工程に独立した AI 連鎖を組み込んでいる。本提出物は、アドネス株式会社の採用評価タスクとして1週間で構築した、Claude Code ブートキャンプの販売スライド自動生成システムである。

### 設計の3点

| 階層 | こだわった判断 | 詳細 |
|---|---|---|
| 上流 | grill-me 対話で暗黙知を AI に取り込む | フォーム入力ではなく、AIが推奨案付きで1問ずつ質問する形 |
| 中流 | 4層データレイヤー分離で「差し替え可能性」を担保 | `sourceDraft / extractedParts / slidePlan / slides` を並列保持 |
| 下流 | HTML→PNG 思想をドキュメントの図解にも適用 | スライド生成と同じ手法で本書の連鎖図画像も生成 |
